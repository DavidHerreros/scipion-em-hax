...
class JaxProtFlexibleAlignmentHetSiren(ProtAnalysis3D, ProtFlexBase):
    ...
    def _defineParams(self, form):
        ...
        form.addParam(
            ...
            params.BooleanParam,
            default=False,
            label="Fine tune previous network?",
        ...
        group.addParam(
            ...
            params.IntParam,
            default=8,
            label="Number of images in batch",
    ...
    def trainingPredictStep(self):
        ...
        args = (
            "--md %s --sr %f --lat_dim %d --epochs %d --batch_size %d --learning_rate %s --output_path %s "
    ...
    def _validate(self):
        ...
        if mask is not None:
            ...
            if not np.all(np.logical_and(data >= 0, data <= 1)):
                errors.append(
                    "Mask provided is not binary. Please, provide a binary mask"
                )
