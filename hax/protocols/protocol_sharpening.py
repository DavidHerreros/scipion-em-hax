...
class JaxProtSharpeningHetSiren(ProtAnalysis3D, ProtFlexBase):
    ...
    def _defineParams(self, form):
        ...
        group.addParam('nIterations', params.IntParam, default=20000,
                       label='Number of iterations in fitting',
                       help='This specific number will give you the length of the total gaussian fitting optimization. The greater the value, '
                            'the greater the convergence but accounting for the risk of overfitting. A tested number for good convergence is the '
                            'one given by default.')
        group.addParam('maxGaussians', params.IntParam, default=100000,
                       label='Maximum number of gaussians',
                       help='Limit number of gaussians to provide the network to work in the total gaussian fitting optimization. The greater the value, '
                            'the more freedom it is given to the network, taking into account that it may lead to overfitting.')
    ...
    def writeMetaDataStep(self):
        ...
        correctionFactor = self.inputVolume.get().getXDim() / vol_mask_dim
        ...
        if self.inputVolumeMask.get():
            mask_data = ImageHandler(fnVolMask).getData()
            protein_voxels = volume[mask_data > 0]
            p_min = np.percentile(protein_voxels, 1)
            p_max = np.percentile(protein_voxels, 99.9)
        else:
            p_min, p_max = np.percentile(volume, 1), np.percentile(volume, 99.9)
        vol_norm = (volume - p_min) / (p_max - p_min + 1e-8)
        volume = np.clip(vol_norm, 0.0, 1.0)
        if self.inputVolumeMask.get():
            mask_data = ImageHandler(fnVolMask).getData()
            volume = vol_norm * mask_data

        ImageHandler(fnVol).write(volume, fnVol, sr=sr)
        particles, angles = ImageHandler().generateProjections(num_projections=self.numProjections.get(), degrees=True, pad=0, useFourier=True,
                                                             volume=volume, n_jobs=self.numberOfThreads.get())
    ...
    def trainingPredictStep(self):
        ...
        if not os.path.isdir(out_path):
            os.mkdir(out_path)
        nIterations = self.nIterations.get()
        ...
        sr = correctionFactor * self.inputVolume.get().getSamplingRate()
        args = ("--md %s --vol %s --sr %f --n_iter %d --max_gaussians %d --lat_dim %d "
                "--epochs %d --batch_size %d --learning_rate %s --output_path %s --denoising_strength %f ") \
               % (md_file, vol_file,  sr, nIterations, maxGaussians, latDim,
